...
const BreathingSession: React.FC = () => {
  ...
  const { user } = useAuth();
  const isRescueMode = searchParams.get('rescue') === 'true';
  ...
  const getInitialProtocol = (): BreathingProtocol => {
    ...
    return (
      PROTOCOLS.find((p) => p.name === settings.breathingPattern.type) ||
      PROTOCOLS[0]
    );
  };

  const [selectedProtocol, setSelectedProtocol] = useState<BreathingProtocol>(
    getInitialProtocol()
  );
  const [customProtocol, setCustomProtocol] = useState<BreathingProtocol>(
    getInitialProtocol()
  );
  const [isCustom, setIsCustom] = useState(
    settings.breathingPattern.type === "custom"
  );
  ...
  // Update component state when settings change (e.g., after navigation from CustomizeSession)
  useEffect(() => {
    // Don't override rescue mode settings
    if (isRescueMode) return;
    
    const newProtocol = getInitialProtocol();
    const newIsCustom = settings.breathingPattern.type === "custom";
    const newTimer = settings.duration * 60;

    // Update protocol states
    setSelectedProtocol(newProtocol);
    setCustomProtocol(newProtocol);
    setIsCustom(newIsCustom);
    setSelectedTimer(newTimer);
  }, [settings, isRescueMode]);
  ...
  const startPhase = (phase: Phase) => {
    // Clear any existing phase interval to prevent overlapping
    ...
    // Reset phase progress for clean transition
    setPhaseProgress(0);
    setPhaseTimeRemaining(Math.ceil(duration / 1000));
    ...
    phaseIntervalRef.current = setInterval(() => {
      ...
      const easedProgress =
        phase === "inhale" || phase === "exhale"
          ? 0.5 * (1 - Math.cos(Math.PI * progress)) // Smooth sine wave easing
          : progress; // Linear for hold phases

      setPhaseProgress(easedProgress);
      setPhaseTimeRemaining(remaining);

      if (progress >= 1) {
        // Clear current interval before transitioning
        if (phaseIntervalRef.current) {
          clearInterval(phaseIntervalRef.current);
          phaseIntervalRef.current = null;
        }
  ...
  const resumeSession = () => {
    ...
    pausedTimeRef.current +=
      pauseEndTime - (startTimeRef.current + pausedTimeRef.current);
    startTimer();

    // Reset phase progress to prevent visual glitches when resuming
    setPhaseProgress(0);
  ...
  const getBreathingRingScale = (): number => {
    ...
    // If phase duration is 0, return appropriate static scale
    if (phaseDuration === 0) {
      return currentPhase === "hold1" ? 1.0 : 0.5;
    }
    
    // Calculate progress based on countdown timer for perfect synchronization
    const timeProgress = 1 - (phaseTimeRemaining / phaseDuration);
    
    if (currentPhase === "inhale") {
      // Ring expands from 0.5 to 1.0 as countdown goes from full duration to 0
      return 0.5 + timeProgress * 0.5;
    } else if (currentPhase === "exhale") {
      // Ring shrinks from 1.0 to 0.5 as countdown goes from full duration to 0
      return 1.0 - timeProgress * 0.5;
    }
    
    // Hold phases stay at their respective sizes
    return currentPhase === "hold1" ? 1.0 : 0.5;
  };
  ...
  // Set default rescue mode settings without auto-starting
  useEffect(() => {
    if (isRescueMode && !isSessionActive && !showCompletion) {
      // Set default rescue mode settings - 4-7-8 breathing pattern
      const rescueProtocol = PROTOCOLS[1]; // 4-7-8-0 pattern
      setSelectedProtocol(rescueProtocol);
      setCustomProtocol(rescueProtocol); // Ensure custom protocol is also set
      setIsCustom(false);
      setSelectedTimer(120); // 2 minutes default
    }
  }, [isRescueMode, isSessionActive, showCompletion]);
    ...
    return (
      <div className="min-h-screen bg-forest-bg-1 flex items-center justify-center p-4">
        <div className="forest-card backdrop-blur-sm rounded-2xl p-8 text-center">
          ...
          <p className="text-forest-text-secondary">
    ...
    return (
      <div className="min-h-screen bg-forest-bg-1 flex flex-col items-center justify-center p-4">
        ...
        <div className="relative mb-8">
          <div
            className="w-64 h-64 rounded-full border-4 border-forest-accent/60 transition-all duration-1000 ease-in-out"
            style={{
              transform: `scale(${getBreathingRingScale()})`,
              boxShadow: `0 0 ${
                40 * getBreathingRingScale()
              }px rgba(139, 195, 74, 0.4)`,
              filter: `blur(${(1 - getBreathingRingScale()) * 2}px)`,
            }}
          />
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <div className="text-3xl font-bold text-forest-text-primary transition-all duration-500 ease-in-out">
                {getPhaseLabel(currentPhase)}
              </div>
              <div className="text-forest-accent transition-all duration-500 ease-in-out text-5xl font-bold tracking-wider drop-shadow-lg">
                {getPhaseDuration(currentPhase) === 0
                  ? "—"
                  : phaseTimeRemaining}
              </div>
              {getPhaseDuration(currentPhase) > 0 && (
                <div className="text-forest-text-secondary text-sm mt-1 transition-all duration-300 ease-in-out">
                  seconds remaining
                </div>
              )}
            </div>
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-forest-text-primary">
                Breathing Session
              </h1>
              {isRescueMode && (
                <div className="text-sm text-forest-accent font-medium">
                  🚨 Rescue Mode - Quick Relief Ready
                </div>
              )}
            </div>
          </div>
        ...
        {/* Protocol Selection */}
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-forest-text-primary mb-3">
            Choose Protocol
          </h2>
            ...
            {PROTOCOLS.map((protocol) => (
              ...
              >
                ...
                <div className="text-sm opacity-80">
                  {protocol.inhale}-{protocol.hold1}-{protocol.exhale}-
                  {protocol.hold2}
                </div>
              </button>
            ))}
        ...
        {isCustom && (
          <div className="mb-6 p-4 forest-card rounded-xl border border-forest-bg-2">
            <h3 className="text-forest-text-primary font-medium mb-4">
              Custom Timing
            </h3>
            <div className="space-y-4">
              {(["inhale", "hold1", "exhale", "hold2"] as const).map(
                (phase) => (
                  <div key={phase}>
        ...
        <div className="mb-6">
          ...
          <div className="p-4 forest-card rounded-xl border border-forest-bg-2">
            <div className="text-forest-text-primary font-medium mb-2">
              Selected:{" "}
              {settings.backgroundNoise.charAt(0).toUpperCase() +
                settings.backgroundNoise.slice(1)}{" "}
              Noise
            </div>
          ...
          <p className="text-sm text-forest-text-secondary mt-2">
            Your selected background audio will automatically start when you
            begin your session.
          </p>
        </div>
